...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .card-section:not(:last-child)::after { right: 20%; top: auto; bottom: 0; height: 1px; width: 60%; background: linear-gradient(to right, transparent, #30363d, transparent); }
...
def render_drift_summary(drift_metrics):
    ...
    try:
        ...
        if is_drifted:
            status_html = f"""<div class="drift-status-alert"><h3 style="color: #FF4B4B; margin: 0;">⚠️ DATA DRIFT DETECTED</h3><p style="color: #c9d1d9; margin: 10px 0 0 0;">{drift_share} out of {total_columns} features have significantly shifted</p></div>"""
        else:
            status_html = f"""<div class="drift-status-ok"><h3 style="color: #17B794; margin: 0;">✅ DATA STABLE</h3><p style="color: #c9d1d9; margin: 10px 0 0 0;">No significant drift detected in your data</p></div>"""
...
def render_drift_table(drift_metrics):
    ...
    try:
        ...
        if drift_data:
            drift_df = pd.DataFrame(drift_data).sort_values('Drift Score', ascending=False)
            def highlight_drift(row):
                if row['Drift Detected'] == 'Yes': return ['background-color: #2d1515; color: #FF4B4B'] * len(row)
                else: return ['background-color: #0d2818; color: #17B794'] * len(row)
            st.dataframe(drift_df.style.apply(highlight_drift, axis=1), use_container_width=True, height=400)
    except Exception as e:
    ...
    try:
        ...
        for key, val in classification_metrics.items():
            if isinstance(val, dict):
                if 'roc_auc' in str(val).lower(): metrics_data['ROC AUC'] = val.get('value', val.get('roc_auc', 0))
                if 'precision' in str(val).lower() and 'recall' not in key.lower(): metrics_data['Precision'] = val.get('value', val.get('precision', 0))
                if 'recall' in str(val).lower(): metrics_data['Recall'] = val.get('value', val.get('recall', 0))
                if 'f1' in str(val).lower(): metrics_data['F1 Score'] = val.get('value', val.get('f1', 0))
                if 'accuracy' in str(val).lower(): metrics_data['Accuracy'] = val.get('value', val.get('accuracy', 0))
        if metrics_data:
            cols = st.columns(len(metrics_data))
            for idx, (metric_name, value) in enumerate(metrics_data.items()):
                if isinstance(value, (int, float)):
                    color = '#17B794' if value > 0.8 else '#EEB76B' if value > 0.6 else '#FF4B4B'
                    metric_html = f"""<div class="metric-card"><div class="metric-card-value" style="color: {color};">{value:.3f}</div><div class="metric-card-label">{metric_name}</div></div>"""
                    with cols[idx]: st.markdown(metric_html, unsafe_allow_html=True)
        else: st.info("No classification metrics extracted")
    except Exception as e:
...
def pie_chart(the_df, column):
    ...
    fig.update_traces(textfont={"size": 16, "family": "arial", "color": "#fff"}, hovertemplate="Label:%{label}<br>Frequency: %{value:0.4s}<br>Percentage: %{percent}", marker=dict(line=dict(color='#000000', width=0.5)), pull=pulls)
    return fig
...
def run_groq_consultant(employee_name, department, situation, solution, budget):
    ...
    template = """You are an expert HR Consultant. **Employee:** {employee_name} ({department}) **Situation (HR View):** {situation} **Technical Root Cause (Internal):** {root_cause} **Proposed Solution:** {action_description} **Estimated Cost:** {cost_str} **Task:** Write a polite, professional, and empathetic email draft from the HR Manager to the employee. Acknowledge their value. Gently address the situation. Propose the solution clearly. **Tone:** Professional, Supportive."""
    ...
    with st.spinner("Drafting your message..."):
        try:
            ...
            if "rate_limit" in str(e).lower() or "429" in str(e): st.warning("⏳ **AI is busy right now.** The servers are overloaded. Please wait 30 seconds and try again.")
            else: st.error(f"Error generating draft: {e}")
...
def main():
    ...
    if st.secrets.get("DEBUG_MODE", "false").lower() == "true":
        with st.sidebar: st.caption(f"Evidently version: {EVIDENTLY_VERSION}")
    ...
    if 'is_global' in st.session_state and st.session_state['is_global']:
        ...
        def load_data_and_train_model(_model_version="v4_calibrated"):
            ...
            preprocessor = ColumnTransformer(transformers=[('num', 'passthrough', numerical_features), ('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False), categorical_features)])
    ...
    if page == "⚙️ Global Setup":
        ...
        if uploaded_file is not None:
            try:
                ...
                st.markdown("---"); st.markdown("### 🔍 Step 2.5: Data Quality Check (Evidently AI)")
                ...
                if st.button("🔬 Run Data Quality Report", type="secondary", key="quality_check"):
                    with st.spinner("Analyzing data quality..."):
                        try:
                            quality_df = new_df.copy(); quality_df[target_col] = quality_df[target_col].apply(lambda x: 1 if x == left_value else 0)
                            ...
                            if quality_report is None:
                                ...
                                quality_metrics = extract_drift_metrics(quality_report); st.success("✅ Data Quality Report Generated")
                                quality_data = quality_metrics.get('DataQualityMetric', {})
                                col_q1, col_q2, col_q3, col_q4 = st.columns(4)
                                with col_q1: st.metric("Total Rows", quality_data.get('rows_count', len(new_df)))
                                with col_q2: st.metric("Total Columns", quality_data.get('columns_count', len(new_df.columns)))
                                with col_q3:
                                ...
                                if missing > 0:
                                    with st.expander("📋 Missing Values by Column"):
                                        ...
                                        if missing_by_col:
                                            missing_df = pd.DataFrame([{'Column': k.replace('_', ' ').title(), 'Missing %': f"{v*100:.1f}%"} for k, v in missing_by_col.items() if v > 0])
                                            st.dataframe(missing_df, use_container_width=True)
                        except Exception as e: st.error(f"Data quality check failed: {e}")
        ...
        with st.expander("📊 Quick Data Health Check (Evidently AI)", expanded=False):
            ...
            if st.button("🔍 Run Quick Drift Check", type="secondary", key="quick_drift"):
                with st.spinner("Checking for data drift..."):
                    try:
                        reference_data = df.sample(frac=0.8, random_state=42); current_data = df.drop(reference_data.index)
                        ...
                        if drift_report is None:
                            ...
                            drift_detected = 0; total_cols = len(numeric_cols)
                            for col in numeric_cols[:10]:
                                ref_mean = reference_data[col].mean(); cur_mean = current_data[col].mean(); ref_std = reference_data[col].std()
                                if ref_std > 0:
                                    z_score = abs(ref_mean - cur_mean) / ref_std
                                    if z_score > 2: drift_detected += 1
                            if drift_detected > 0: st.warning(f"⚠️ Basic check: {drift_detected}/{total_cols} features may have shifted")
                            else: st.success("✅ Basic check: Data appears stable")
                        else:
                            drift_metrics = extract_drift_metrics(drift_report)
                            is_drifted, drift_cols, total_cols = render_drift_summary(drift_metrics)
                            if not is_drifted: st.success("✅ Your data is stable. No significant drift detected between the reference and current periods.")
                            else: st.warning(f"⚠️ Drift detected in {drift_cols} features. Consider retraining your model.")
                            with st.expander("📊 Detailed Drift Table"): render_drift_table(drift_metrics)
                    except Exception as e: st.error(f"Drift check failed: {e}")
        
        st.markdown("---"); st.markdown("### 📄 Employee Data Snapshot")
    ...
    if page == "Predict Attrition":
        ...
        with st.expander("🧪 Model Diagnostics (Verification)", expanded=False):
            ...
            with c_test1:
                if st.button("Test with Employee who Left"):
                    sample = df[df['left'] == 1].iloc[0]; test_df = sample.drop('left').to_frame().T
                    raw_prob = pipeline.predict_proba(test_df)[0][1]; calibrated_prob = calibrate_probability(raw_prob); pred = 1 if calibrated_prob >= 0.5 else 0
            ...
            with c_test2:
                if st.button("Test with Employee who Stayed"):
                    sample = df[df['left'] == 0].iloc[0]; test_df = sample.drop('left').to_frame().T
                    raw_prob = pipeline.predict_proba(test_df)[0][1]; calibrated_prob = calibrate_probability(raw_prob); pred = 1 if calibrated_prob >= 0.5 else 0
        ...
        st.markdown("---")
        feature_columns = [c for c in df.columns if c != 'left']; is_default_data = 'satisfaction_level' in feature_columns 

        with st.form("Predict_value_form"):
            st.markdown("##### 👤 Employee Profile"); input_data = {}
            if is_default_data:
                ...
                for i, col in enumerate(feature_columns):
                    with cols[i%2]:
                        if df[col].dtype == 'object' or df[col].dtype.name == 'category': input_data[col] = st.selectbox(col.replace('_', ' ').title(), df[col].unique())
                        else:
        ...
        if predict_button:
            ...
            with st.spinner('AI is analyzing...'): sleep(1)
            input_df = input_df[feature_columns] 
            raw_probas = pipeline.predict_proba(input_df)[0]; calibrated_stay = calibrate_probability(raw_probas[0], temperature=0.55); calibrated_leave = 1 - calibrated_stay; prediction = 1 if calibrated_leave >= 0.5 else 0
            st.session_state.prediction_result = prediction; st.session_state.input_df = input_df; st.session_state.prediction_probas = [calibrated_stay, calibrated_leave]

        if st.session_state.prediction_result is not None:
            st.markdown("---")
            stay_prob = st.session_state.prediction_probas[0]; leave_prob = st.session_state.prediction_probas[1]
            stay_percent = int(round(stay_prob * 100)); leave_percent = int(round(leave_prob * 100))
            if st.session_state.prediction_result == 1: result_text = "LEAVE"; result_class = "result-leave"; highlight_class = "highlight-leave"; stay_indicator = "indicator-leave"; leave_indicator = "indicator-leave"
            else: result_text = "STAY"; result_class = "result-stay"; highlight_class = "highlight-stay"; stay_indicator = "indicator-stay"; leave_indicator = "indicator-stay"
            
            st.markdown(f"""<div class="prediction-card {highlight_class}"><div class="card-section card-section-first"><div class="card-label">Employee is Likely to</div><div class="card-result {result_class}">{result_text}</div></div><div class="card-section"><div class="card-label">Probability to Stay</div><div class="card-percentage percentage-stay">{stay_percent}%</div><div class="card-indicator {stay_indicator}"></div></div><div class="card-section"><div class="card-label">Probability to Leave</div><div class="card-percentage percentage-leave">{leave_percent}%</div><div class="card-indicator {leave_indicator}"></div></div></div>""", unsafe_allow_html=True)
                    ...
                    with st.spinner("Simulating retention strategies..."):
                        try:
                            ...
                            if not continuous_features: st.error("No numerical columns found for simulation.")
                                ...
                                original = query_instance.iloc[0]; scenarios_html = []
                                ...
                                col_s1, col_s2, col_s3 = st.columns(3); cols_list = [col_s1, col_s2, col_s3]
    ...
    if page == "Why They Leave":
        ...
        st.write("---"); analyze_why_people_leave(df)
    ...
    if page == "Budget Planner":
        ...
        st.markdown("""<div class="custom-card" style="border-left: 5px solid #FF4B4B; background: linear-gradient(to right, #2d1515 0%, #1c2128 100%);"><h3 style="color: #FF4B4B; margin-top: 0;">💸 Phase 1: The Burn Rate</h3><p style="color: #e6edf3; font-size: 1rem;"><strong>The Question:</strong> "How much money are we actually losing, and where?"</p><p style="color: #8b949e; font-size: 0.9rem; margin-bottom: 0;'>CFOs speak in money. Translating "23% attrition" into hard numbers immediately gets budget approval.</p></div>""", unsafe_allow_html=True)
        if 'Department' in df.columns:
            salary_map = {'low': 400000, 'medium': 600000, 'high': 900000}
            df_cost = df.copy(); df_cost['annual_salary'] = df_cost['salary'].map(salary_map) if 'salary' in df_cost.columns else 500000; df_left = df_cost[df_cost['left'] == 1].copy()
            if len(df_left) > 0:
                if 'time_spend_company' in df_left.columns:
                    tenure_multiplier = (1 + (df_left['time_spend_company'] * 0.10)).clip(upper=2.0); df_left['total_cost'] = df_left['annual_salary'] * 0.5 * tenure_multiplier
                else: df_left['total_cost'] = df_left['annual_salary'] * 0.75
                dept_costs = df_left.groupby('Department').agg(employees_left=('left', 'count'), total_cost=('total_cost', 'sum'), avg_salary=('annual_salary', 'mean'), avg_tenure=('time_spend_company', 'mean') if 'time_spend_company' in df_left.columns else pd.Series(3.0)).reset_index().sort_values('total_cost', ascending=False)
                grand_total = dept_costs['total_cost'].sum(); total_left = dept_costs['employees_left'].sum(); avg_exit_cost = grand_total / total_left
                ...
                with st.expander("📐 How we calculate this (Click to verify)"): st.markdown("""We use the **Standard HR Industry Formula** for turnover cost:\n> `Cost = 50% of Annual Salary × (1 + Years of Tenure × 10%)`\n*   **Base Replacement Cost:** 50% of salary (covers recruiting, onboarding, admin).\n*   **Experience Penalty:** +10% for every year of experience the employee had (captures institutional knowledge loss).\n*   **Cap:** The multiplier is capped at 2.0x (200% of salary) for very senior employees.\n*Example:* A 5-year employee earning ₹6L costs `0.5 × 6L × 1.5 = ₹4.5L` to replace.""")
                fig_cost = px.bar(dept_costs, x='Department', y='total_cost', title="True Cost of Turnover by Department", template="plotly_dark", color='total_cost', color_continuous_scale=['#17B794', '#FF4B4B'], height=450)
                fig_cost.update_layout(yaxis_title="Total Loss (₹)", yaxis=dict(tickformat=',.0f'), xaxis={'categoryorder': 'total descending'}, xaxis_title="", showlegend=False); custome_layout(fig_cost, title_size=24, showlegend=False); st.plotly_chart(fig_cost, use_container_width=True)
        ...
        st.markdown("""<div class="custom-card" style="border-left: 5px solid #17B794; background: linear-gradient(to right, #0d2818 0%, #1c2128 100%);"><h3 style="color: #17B794; margin-top: 0;">🛡️ Phase 2: The ROI Optimizer</h3><p style="color: #e6edf3; font-size: 1rem;"><strong>The Question:</strong> "Who should we save with our limited budget?"</p><p style="color: #8b949e; font-size: 0.9rem; margin-bottom: 0;'>We use Mathematical Optimization (MILP) to find the exact combination of employees that yields maximum ROI. It's cheaper to give a 10% raise than to replace them.</p></div>""", unsafe_allow_html=True)
        ...
        if optimize_btn:
            with st.spinner("🧮 Running Mathematical Optimization..."):
                X = df.drop('left', axis=1); raw_probs = pipeline.predict_proba(X)[:, 1]; probas = calibrate_probability_array(raw_probs, temperature=0.55)
                opt_df = df.copy(); opt_df['risk'] = probas; high_risk_df = opt_df[opt_df['risk'] > 0.5].copy()
                if len(high_risk_df) == 0: st.success("🎉 Great news! The AI predicts workforce is highly stable.")
                else:
                    if 'salary' in df.columns: high_risk_df['salary_val'] = high_risk_df['salary'].map(salary_map)
                    else: high_risk_df['salary_val'] = 500000 
                    high_risk_df['cost_to_retain'] = high_risk_df['salary_val'] * 0.10; high_risk_df['expected_loss'] = high_risk_df['risk'] * (high_risk_df['salary_val'] * 0.50); high_risk_df['net_savings'] = high_risk_df['expected_loss'] - high_risk_df['cost_to_retain']
                    candidates = high_risk_df[high_risk_df['net_savings'] > 0].copy()
                    if len(candidates) == 0: st.warning("⚠️ Based on current risk probabilities, a 10% raise is not mathematically justifiable for the at-risk group.")
                    else:
                        n = len(candidates); c = -candidates['net_savings'].values; A = np.array([candidates['cost_to_retain'].values]); b = np.array([budget]); integrality = np.ones(n) 
                        try: res = milp(c=c, constraints=LinearConstraint(A, lb=-np.inf, ub=b), integrality=integrality)
                        except Exception as e: st.error(f"Calculation Error: {e}"); res = None
                        if res and res.success:
                            selected_indices = np.where(res.x == 1)[0]; selected_employees = candidates.iloc[selected_indices]
                            total_investment = selected_employees['cost_to_retain'].sum(); total_savings = selected_employees['net_savings'].sum()
                            ...
                            st.markdown("### 📋 Target List"); st.caption("These employees have been mathematically proven to be cheaper to retain than to replace.")
                            display_df = selected_employees[['Department', 'salary', 'risk', 'cost_to_retain', 'net_savings']].copy()
                            display_df.rename(columns={'salary': 'Salary Tier', 'risk': 'AI Risk %', 'cost_to_retain': 'Cost to Retain (₹)', 'net_savings': 'Net Savings (₹)'}, inplace=True)
                        ...
                        else: st.error("❌ Optimization failed. Budget may be too low to save anyone.")
                            
    if page == "AI Assistant":
        st.header("🤖 AI Assistant")
        st.markdown("<p style='color: #9ca3af;'>Tools to ensure reliability and simplify communication.</p>", unsafe_allow_html=True)
        st.markdown("### ✍️ Draft Retention Communication"); st.write("Select a scenario, and we'll draft a message for you.")
    ...
    if page == "💬 HR Chatbot":
        ...
        with chat_container:
            st.markdown("""<div class="chat-container"><div class="chat-header"><h3 style="margin:0; color: #17B794;">RetainAI Copilot</h3><small style="color: #8b949e;">Powered by Groq LLM + Internal Data</small></div><div class="chat-messages" id="chat-messages">""", unsafe_allow_html=True)
    ...
    if page == "AI Research Lab":
        ...
        with tab1:
            st.subheader("Algorithm Performance Comparison"); st.caption("Using Evidently AI for comprehensive classification metrics")
            if st.button("Run Benchmark", type="primary", key="run_benchmark"):
                with st.spinner("Training competing models..."):
                    ...
                    st.markdown("---"); st.markdown("### 📊 Evidently AI Classification Report (LightGBM)"); st.caption("Detailed classification quality analysis by Evidently AI")
                    try:
                        reference_data = X_train_ref.copy(); reference_data['left'] = y_train.values
                        current_data = X_test_cur.copy(); current_data['left'] = y_test.values; current_data['prediction'] = y_pred_lgbm
                        classification_report = run_classification_performance_report(reference_data, current_data, y_pred_lgbm)
                        if classification_report is None: st.warning("⚠️ Evidently AI classification report not available (version compatibility). Using sklearn metrics shown above.")
                        else:
                            classification_metrics = extract_drift_metrics(classification_report); render_classification_metrics(classification_metrics)
                            with st.expander("📋 Detailed Metrics Breakdown"):
                                for metric_name, metric_data in classification_metrics.items():
                                    st.markdown(f"**{metric_name}**")
                                    if isinstance(metric_data, dict):
                                        for k, v in metric_data.items():
                                            if isinstance(v, float): st.write(f"  • {k}: `{v:.4f}`")
                                            else: st.write(f"  • {k}: `{v}`")
                                    st.markdown("---")
                    except Exception as e: st.warning(f"Evidently classification report failed: {e}")
            ...
            if 'Department' not in df.columns: st.warning("Department column not found in this dataset. Cannot run deep dive.")
                ...
                if st.button("Generate Department Strategy", type="primary"):
                    with st.spinner("Analyzing departmental dynamics..."):
                        ...
                        c_m1.metric(f"{selected_dept_name} Workforce", f"{dept_count} Employees"); c_m2.metric("Attrition Rate", f"{dept_attrition:.1f}%", delta=f"{delta:+.1f}% vs Avg", delta_color="inverse" if delta>0 else "normal"); c_m3.metric("Risk Level", "High" if dept_attrition > 20 else "Moderate" if dept_attrition > 10 else "Low")
                        st.markdown("---"); shap_vals, X_proc_df = get_shap_explanations(pipeline, df)
                        target_col = None; dept_normalized = selected_dept_name.lower().replace('_', '').replace(' ', '')
                        for col in X_proc_df.columns:
                            if dept_normalized in col.lower().replace('_', '').replace(' ', ''): target_col = col; break
                        if not target_col: st.error(f"Could not find data for {selected_dept_name} in the model features.")
                            ...
                            if dept_mask.sum() == 0: st.warning(f"Not enough data to analyze {selected_dept_name} specifically.")
                                ...
                                def get_driver_advice(feature_raw):
                                    ...
                                    else: return f"Monitor {feature_raw.replace('_', ' ').title()}", f"AI identified {feature_raw.replace('_', ' ').title()} as a key differentiator. Investigate department-specific policies related to this metric.", "🔍"
                                c1, c2, c3 = st.columns(3); cols = [c1, c2, c3]
        ...
        with tab3:
            st.subheader("🛡️ AI Disruption Defense"); st.caption("Prove to leadership that reskilling is cheaper than mass layoffs.")
            st.error("**The Fear:** CEO asks, 'Can we just replace half the team with AI tools?'")
            st.success("**The Reality:** AI replaces *tasks*, not jobs. And layoffs cost way more than you think."); st.write("")
            if 'Department' in df.columns and 'satisfaction_level' in df.columns:
                ...
                for dept in df['Department'].unique():
                    dept_data = df[df['Department'] == dept]; avg_sat = dept_data['satisfaction_level'].mean()
            ...
            st.markdown("---"); st.markdown("""<div class="custom-card" style="border-left: 5px solid #9ca3ca; background: linear-gradient(to right, #151d28 0%, #1c2128 100%);"><h4 style="color: #9ca3ca; margin-top: 0;">🧠 The 'Proof of Work' Defense Strategy</h4><p style="color: #e6edf3; font-size: 1rem; font-weight: 600; margin-bottom: 10px;">What to say when the CEO asks: "Can we just use AI to cut headcount?"</p><p style="color: #c9d1d9; font-size: 0.95rem; line-height: 1.6; margin-bottom: 10px;">"We shouldn't try to compete with AI on speed; we must compete on strategy. AI can write code in 10 seconds, but it takes a human 3 weeks to understand the business context, manage stakeholder politics, and ensure compliance. If we lay off 200 people to save ₹12 Cr, we lose their domain expertise. If we give 50 people AI tools, we increase their capacity by 30%, allowing us to <strong>absorb the workload of the 200 people who left last year</strong> without losing institutional knowledge."</p><p style="color: #8b949e; font-size: 0.85rem; margin-top: 10px; margin-bottom: 0;"><strong>Bottom Line:</strong> AI doesn't replace jobs; it replaces tasks. Our goal isn't to have fewer people; it's to have highly utilized people. We should pitch AI as a way to make our existing team 30% faster, not as a way to fire people.</p></div>""", unsafe_allow_html=True)
            ...
            st.markdown("---")
            severance_cost = (avg_salary / 12) * 3 * num_employees; new_hire_cost = (avg_salary * 2) * (num_employees * 0.1); total_layoff_cost = severance_cost + new_hire_cost
            training_cost = (avg_salary / 12) * 1.5 * num_employees; productivity_dip = (avg_salary / 12) * 1 * num_employees; total_reskill_cost = training_cost + productivity_dip; savings = total_layoff_cost - total_reskill_cost
            c_res1, c_res2, c_res3 = st.columns(3)
            c_res1.metric("Total Cost of Layoffs", f"₹{total_layoff_cost/10000000:.2f} Cr"); c_res2.metric("Total Cost of Reskilling", f"₹{total_reskill_cost/10000000:.2f} Cr"); c_res3.metric("Money Saved by Reskilling", f"₹{savings/10000000:.2f} Cr", delta="Reskill wins")
            st.markdown("---")
        ...
        with tab4:
            ...
            st.markdown("""<div class="custom-card" style="border-left: 5px solid #17B794; background: linear-gradient(to right, #0d2818 0%, #1c2128 100%);"><h4 style="color: #17B794; margin-top: 0;">🔍 Evidently AI Monitoring</h4><p style="color: #e6edf3; font-size: 1rem; margin-bottom: 0;">Detect when your data distribution changes. Data drift can silently degrade model performance over time.</p></div>""", unsafe_allow_html=True)
            if EVIDENTLY_VERSION == "minimal": st.warning("""⚠️ **Evidently AI Limited Mode**\n\nThe installed version of Evidently AI has compatibility issues. Using fallback drift detection based on basic statistical methods.\n\nTo get full Evidently features, try upgrading: `pip install evidently==0.4.22`""")
            st.markdown("### 📊 What is Data Drift?")
            st.markdown("""<p style='color: #c9d1d9; line-height: 1.6;'>Data drift occurs when the statistical properties of your input data change over time. For example:<br><br>• <strong>Average satisfaction drops</strong> from 0.65 to 0.55 company-wide<br>• <strong>Monthly working hours increase</strong> due to new project demands<br>• <strong>Department composition changes</strong> after a reorg<br><br>When drift occurs, your model's predictions become less reliable because it was trained on "old" data patterns.</p>""", unsafe_allow_html=True)
            st.markdown("---"); st.markdown("### 🧪 Run Drift Analysis")
            col_drift1, col_drift2 = st.columns(2)
            with col_drift1:
                drift_scenario = st.selectbox("Select Analysis Scenario", options=["Train vs Test Split (Default)", "First 50% vs Last 50% (Time Simulation)", "Random Sample Comparison", "High Risk vs Low Risk Employees"])
            with col_drift2:
                if EVIDENTLY_VERSION != "minimal": drift_threshold = st.slider("Drift Sensitivity Threshold", min_value=0.01, max_value=0.20, value=0.05, step=0.01, help="Lower values = more sensitive to small changes")
                else: st.info("Using default threshold: 2 standard deviations")
            
            if st.button("🔍 Run Data Drift Analysis", type="primary", key="run_drift_analysis"):
                with st.spinner("Running drift analysis..."):
                    try:
                        full_data = df.copy()
                        if drift_scenario == "Train vs Test Split (Default)": reference_df = full_data.sample(frac=0.8, random_state=42); current_df = full_data.drop(reference_df.index); scenario_desc = "Training (80%) vs Test (20%) split"
                        elif drift_scenario == "First 50% vs Last 50% (Time Simulation)": mid_point = len(full_data) // 2; reference_df = full_data.iloc[:mid_point]; current_df = full_data.iloc[mid_point:]; scenario_desc = "First half vs Second half (simulating time)"
                        elif drift_scenario == "Random Sample Comparison": reference_df = full_data.sample(frac=0.5, random_state=42); current_df = full_data.sample(frac=0.5, random_state=123); scenario_desc = "Two random 50% samples"
                        elif drift_scenario == "High Risk vs Low Risk Employees":
                            X = full_data.drop('left', axis=1); raw_probs = pipeline.predict_proba(X)[:, 1]; full_data['risk_score'] = calibrate_probability_array(raw_probs)
                            reference_df = full_data[full_data['risk_score'] < 0.5].copy(); current_df = full_data[full_data['risk_score'] >= 0.5].copy(); scenario_desc = "Low Risk (< 50%) vs High Risk (≥ 50%) employees"
                        
                        drift_report = run_data_drift_analysis(reference_df, current_df)
                        st.markdown("---"); st.markdown(f"### 📊 Results: {scenario_desc}")
                        ...
                        if drift_report is None:
                            ...
                            drift_results = []; total_checked = 0; drift_detected_count = 0
                            for col in numeric_cols:
                                ref_mean = reference_df[col].mean(); cur_mean = current_df[col].mean(); ref_std = reference_df[col].std()
                                if ref_std > 0:
                                    z_score = abs(ref_mean - cur_mean) / ref_std; is_drifted = z_score > 2; total_checked += 1
                                    if is_drifted: drift_detected_count += 1
                                    drift_results.append({'Feature': col.replace('_', ' ').title(), 'Drift Detected': 'Yes' if is_drifted else 'No', 'Z-Score': round(z_score, 4), 'Reference Mean': round(ref_mean, 3), 'Current Mean': round(cur_mean, 3), 'Difference': round(cur_mean - ref_mean, 3)})
                            if drift_results:
                                drift_df = pd.DataFrame(drift_results).sort_values('Z-Score', ascending=False)
                                is_drifted = drift_detected_count > 0
                                if is_drifted: st.markdown(f"""<div class="drift-status-alert"><h3 style="color: #FF4B4B; margin: 0;">⚠️ POTENTIAL DRIFT DETECTED</h3><p style="color: #c9d1d9; margin: 10px 0 0 0;">{drift_detected_count} out of {total_checked} features show significant changes (Z-score > 2)</p></div>""", unsafe_allow_html=True)
                                else: st.markdown(f"""<div class="drift-status-ok"><h3 style="color: #17B794; margin: 0;">✅ DATA STABLE</h3><p style="color: #c9d1d9; margin: 10px 0 0 0;">No significant drift detected (all Z-scores < 2)</p></div>""", unsafe_allow_html=True)
                                col_d1, col_d2, col_d3 = st.columns(3)
                                with col_d1: st.metric("Drifted Features", f"{drift_detected_count}/{total_checked}", delta=f"{(drift_detected_count / total_checked * 100) if total_checked > 0 else 0:.1f}%")
                                with col_d2: st.metric("Avg Z-Score", f"{drift_df['Z-Score'].mean():.2f}", delta="Lower is better")
                                with col_d3: st.metric("Overall Status", "⚠️ DRIFTED" if is_drifted else "✅ STABLE")
                                st.markdown("---"); st.markdown("### 📋 Feature-Level Drift Details")
                                def highlight_drift_fallback(row): return ['background-color: #2d1515; color: #FF4B4B'] * len(row) if row['Drift Detected'] == 'Yes' else ['background-color: #0d2818; color: #17B794'] * len(row)
                                st.dataframe(drift_df.style.apply(highlight_drift_fallback, axis=1), use_container_width=True, height=400)
                        else:
                            drift_metrics = extract_drift_metrics(drift_report); is_drifted, drift_cols, total_cols = render_drift_summary(drift_metrics)
                            dataset_drift = drift_metrics.get('DatasetDriftMetric', {}); col_d1, col_d2, col_d3 = st.columns(3)
                            with col_d1: st.metric("Drifted Features", f"{drift_cols}/{total_cols}", delta=f"{(drift_cols / total_cols * 100) if total_cols > 0 else 0:.1f}%")
                            with col_d2: st.metric("Drift Share", f"{dataset_drift.get('drift_share', 0):.2%}", delta="Should be low")
                            with col_d3: st.metric("Overall Status", "⚠️ DRIFTED" if is_drifted else "✅ STABLE")
                            st.markdown("---"); st.markdown("### 📋 Feature-Level Drift Details"); st.caption("Green = No drift detected | Red = Drift detected (above threshold)")
                            render_drift_table(drift_metrics)
                        
                        st.markdown("---"); st.markdown("### 💡 Recommendations")
                        if drift_report is None:
                            if drift_detected_count > 0: st.warning("""**⚠️ Action Required:**\n1. **Investigate drifted features** - Check if the drift is due to data collection issues or genuine changes\n2. **Consider retraining** - If drift is significant and persistent, retrain the model with recent data\n3. **Update monitoring** - Adjust the threshold if the detected changes are expected""")
                            else: st.success("""**✅ No Immediate Action Required**\nYour data distributions are stable. Continue regular monitoring to catch drift early.""")
                        else:
                            if is_drifted: st.warning("""**⚠️ Action Required:**\n1. **Investigate drifted features** - Check if the drift is due to data collection issues or genuine changes\n2. **Consider retraining** - If drift is significant and persistent, retrain the model with recent data\n3. **Update monitoring** - Adjust the drift threshold if the detected changes are expected""")
                            else: st.success("""**✅ No Immediate Action Required**\nYour data distributions are stable. Continue regular monitoring to catch drift early.""")
                        with st.expander("📥 Export Report"):
                            if drift_report is not None:
                                try: st.code(json.dumps(extract_drift_metrics(drift_report), indent=2, default=str), language='json')
                                except: pass
                            else: st.dataframe(drift_df, use_container_width=True)
                    except Exception as e:
                        st.error(f"Drift analysis failed: {e}")
                        with st.expander("🔧 Error Details"): import traceback; st.code(traceback.format_exc())
            
            st.markdown("---"); st.markdown("### 📈 Feature Distribution Comparison"); st.caption("Visual comparison of key features between reference and current periods")
            if st.button("📊 Show Distribution Charts", type="secondary", key="show_dist"):
                with st.spinner("Generating distribution charts..."):
                    try:
                        numeric_cols = df.select_dtypes(include=np.number).columns.drop('left', errors='ignore').tolist()
                        if len(numeric_cols) == 0: st.warning("No numeric columns available for comparison")
                        else:
                            reference_df = df.sample(frac=0.8, random_state=42); current_df = df.drop(reference_df.index)
                            for col in numeric_cols[:6]:
                                fig, axes = plt.subplots(1, 2, figsize=(12, 4))
                                axes[0].hist(reference_df[col].dropna(), bins=30, color='#17B794', alpha=0.7, edgecolor='white')
                                axes[0].set_title(f'Reference: {col.replace("_", " ").title()}', color='white', fontsize=12); axes[0].set_facecolor('#161b22'); axes[0].tick_params(colors='white')
                                ...
                                axes[1].set_title(f'Current: {col.replace("_", " ").title()}', color='white', fontsize=12); axes[1].set_facecolor('#161b22'); axes[1].tick_params(colors='white')
                                for spine in axes[1].spines.values(): spine.set_color('#30363d')
                                fig.patch.set_facecolor('#0E1117'); plt.tight_layout(); st.pyplot(fig); plt.close(fig)
                    except Exception as e: st.error(f"Could not generate distribution charts: {e}")
            st.markdown("---"); st.markdown("""<div class="custom-card" style="background: linear-gradient(to right, #1c2128 0%, #21262d 100%); border-left: 4px solid #8b949e;"><h4 style="color: #8b949e; margin-top: 0;">📅 Monitoring Best Practices</h4><ul style="color: #c9d1d9; line-height: 1.8;"><li><strong>Weekly:</strong> Run quick drift check on new data</li><li><strong>Monthly:</strong> Full drift analysis with distribution comparison</li><li><strong>Quarterly:</strong> Model performance review and potential retraining</li><li><strong>After major changes:</strong> Re-org, policy changes, market shifts → Run immediate drift check</li></ul></div>""", unsafe_allow_html=True)
    ...
    if page == "Strategic Roadmap":
        ...
        if st.button("📈 Show Me the 12-Month Projection", type="primary"):
            ...
            raw_risk_scores = pipeline.predict_proba(df.drop('left', axis=1))[:, 1]; total_risk_score = calibrate_probability_array(raw_risk_scores, temperature=0.55).sum()
            monthly_leavers_no_action = total_risk_score / 12.0; monthly_leavers_with_action = monthly_leavers_no_action * (1 - (intervention_efficacy / 100.0))
            ...
            for m in months:
                ...
                temp_bau -= monthly_leavers_no_action + natural_leavers_bau; temp_int -= monthly_leavers_with_action + natural_leavers_int
