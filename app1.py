...
def analyze_why_people_leave(df):
    ...
    # SAFEGUARD: Only run DoWhy if the specific columns exist
    ...
    if all(col in df.columns for col in required_cols):
        ...
        try:
            ...
            st.warning("Could not generate Causal Graph for this specific dataset. Relying on SHAP below.")
    else:
...
def plan_retention_budget(df, pipeline, budget_limit):
    ...
    # SAFEGUARD: If 'salary' column doesn't exist, assign a flat estimated replacement cost
    if 'salary' in df.columns:
        salary_val_map = {'low': 400000, 'medium': 600000, 'high': 900000}
        high_risk_df['annual_salary'] = high_risk_df['salary'].map(salary_val_map)
    else:
        high_risk_df['annual_salary'] = 500000 # Flat ₹5L assumption for unknown datasets
    ...
    A = np.array([candidates['intervention_cost'].values]); b = np.array([budget_limit]); integrality = np.ones(n)
...
def main():
    ...
    # ====================================================================
    # THE GLOBAL ROUTER (Checks if custom data is uploaded)
    # ====================================================================
    if 'is_global' in st.session_state and st.session_state['is_global']:
        ...
        # Mock y_test for tabs that might need it
    ...
    def get_shap_explanations(_pipeline, _df):
        ...
        # Drop non-numeric columns that might sneak in for global datasets
    ...
    def get_retention_strategies(employee_data):
        ...
        # Dynamic fallback for global datasets
    ...
    if page == "⚙️ Global Setup":
        ...
        st.markdown("<p style='color: #9ca3af; margin-bottom: 20px;'>Turn this AI into your company's dedicated assistant. Upload your HR dataset, and the system will automatically retrain itself to understand your specific workforce.</p>", unsafe_allow_html=True)
        ...
        if uploaded_file is not None:
            try:
                ...
                st.success("✅ File uploaded successfully! Here is a preview:")
                st.dataframe(new_df.head())
                
                st.markdown("---"); st.markdown("### Step 1: Identify the Target (Attrition Column)")
                st.write("Please select the column that tells us if the employee left the company.")
                target_col = st.selectbox("Which column indicates if the employee left?", new_df.columns)
                unique_vals = new_df[target_col].unique()
                left_value = st.selectbox(f"In the '{target_col}' column, which value means 'Left'?", unique_vals)
                ...
                with c_type: st.write("**Text Columns (Categorical)**"); st.write(categorical_auto if categorical_auto else "None detected")
                with c_num: st.write("**Number Columns (Numerical)**"); st.write(numerical_auto if numerical_auto else "None detected")
                
                if st.button("🚀 Train Custom AI Model", type="primary"):
                    with st.spinner("🤖 AI is learning your company's data... This takes a minute."):
                        y = new_df[target_col].apply(lambda x: 1 if x == left_value else 0)
                        X = new_df[feature_cols]
                        valid_idx = X.dropna().index
                        X_clean = X.loc[valid_idx]; y_clean = y.loc[valid_idx]
                        ...
                        st.balloons()
                        st.success(f"🎉 Training Complete! Your custom AI achieved **{acc:.1%}** accuracy.")
                        st.info("You can now go to **'Predict Attrition'** or **'Why They Leave'**—they will use YOUR data automatically!")
    ...
    # ====================================================================
    # PAGES (All adapted for global fallbacks where necessary)
    # ====================================================================
    if page == "Home":
        ...
        if 'satisfaction_level' in df.columns:
            avg_satisfaction = df['satisfaction_level'].mean(); col3 = st.columns(1)[0]; col3.metric("Avg. Satisfaction", f"{avg_satisfaction:.2f} / 1.0")
    ...
    if page == "Employee Insights":
        st.header("📉 Employee Data Analysis"); st.write("Explore the workforce demographics to identify patterns.")
    ...
    if page == "Predict Attrition":
        ...
        # DYNAMIC FORM GENERATOR (The Magic Global Feature)
            ...
            for i, col in enumerate(feature_columns):
                with cols[i%2]:
                    if df[col].dtype == 'object' or df[col].dtype.name == 'category':
                        ...
                        # Use min/max from the actual dataset for dynamic sliders
                        min_val = float(df[col].min()); max_val = float(df[col].max())
                        # If too many unique float values, use number_input instead of slider to prevent UI lag
        ...
        if 'prediction_result' not in st.session_state:
            st.session_state.prediction_result = None; st.session_state.input_df = None; st.session_state.prediction_probas = None
                ...
                if st.button("💡 Show Me How to Keep Them", type="primary", key="gen_cf"):
                    with st.spinner("Simulating retention strategies..."):
                        try:
                            ...
                            # Get continuous features dynamically
                            ...
                            if not continuous_features: st.error("No numerical columns found for simulation.")
                                ...
                                cf_df = cf.cf_examples_list[0].final_cfs_df; original = query_instance.iloc[0]
                                scenarios_html = []
                                for i in range(len(cf_df)):
                                    changes = []; cf_row = cf_df.iloc[i]
                                    for col in original.index:
                                        ...
                                        if isinstance(orig_val, float) and abs(orig_val - new_val) > 0.05:
                                            ...
                                            changes.append(f"<div class='action-item action-item-high-effort'>• <strong>{col.replace('_', ' ').title()}</strong>: Change from {orig_val} to {new_val}.</div>")
    ...
    if page == "Why They Leave":
        st.header("🧠 Key Attrition Drivers"); st.write("---")
        ...
        with st.spinner("Analyzing model insights..."):
            try:
                ...
                for idx, row in enumerate(top_3.iterrows()):
                    ...
                    card_html = f"<div class='custom-card' style='text-align: center; height: 100%;'><h3 style='color: #17B794; margin-top: 0;'>{feature}</h3><p style='color: #c9d1d9; font-size: 0.9rem;'>Top {idx+1} Driver</p></div>"
    ...
    if page == "Budget Planner":
        ...
        if optimize_btn:
            ...
            if results:
                ...
                m1, m2 = st.columns(2)
                m1.metric("Investment Needed", f"₹{total_cost:,.0f}"); m2.metric("Projected Savings", f"₹{total_savings:,.0f}", delta="ROI Positive")
    ...
    if page == "AI Research Lab":
        ...
        tab1, tab2 = st.tabs(["📊 Model Benchmarking", "🕵️ Anomaly Detection"])
        with tab1:
